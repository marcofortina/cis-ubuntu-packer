...
echo "Starting LUKS + vTPM binding and initramfs hardening..."
...
# -------------------------
# Install TPM2 tools + Clevis
# -------------------------
echo "Installing TPM2 + Clevis packages..."
apt-get update
apt-get install -y tpm2-tools clevis clevis-luks clevis-initramfs
...
# -------------------------
# Bind LUKS to TPM2 (auto-unlock only on this VM/host)
# -------------------------
echo "Binding LUKS volume to TPM2 via Clevis..."

echo -n "${LUKS_KEY}" | clevis luks bind -d "$LUKS_DEVICE" tpm2 '{}'
...
# -------------------------
echo "Regenerating initramfs for TPM2 auto-unlock..."
update-initramfs -u -k all

echo "LUKS + TPM2 auto-unlock configuration completed successfully."
